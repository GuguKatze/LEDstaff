...
void effectLevel () {
  ...
  if(pitchPacket.pitch < -5 && ballVelocity > -10 && ball > 1){
    ballVelocity--;
  }else if(pitchPacket.pitch > 5 && ballVelocity < 10 && ball  < 142){
    ballVelocity++;
  }else if(pitchPacket.pitch <= 5 && pitchPacket.pitch >= -5){ // bring ball to standstill if vu.pitch is low
    if(ballVelocity < -1){
      ballVelocity++;
